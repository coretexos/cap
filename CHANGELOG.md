...
- SDKs: Go (`github.com/coretexos/cap/cortex/agent/v1` import path with local replace to `generated/go`), Python (asyncio + NATS), and Node/TypeScript (protobufjs loader) aligned to the same contracts.
- Tooling: `tools/make_protos.sh` to generate Go/Python stubs into `generated/`; Python virtualenv support via `PYTHON_BIN`.
